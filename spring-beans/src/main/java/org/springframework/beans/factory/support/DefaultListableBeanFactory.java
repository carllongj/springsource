...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// 复制当前镜像对应的所有BeanDefinitions,允许后续的Bean继续注册
		...
		for (String beanName : beanNames) {
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			// 当Bean为非抽象类,且为单例,并且不是懒加载,则进行实例化Bean
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				// 判断是否获取的为工厂Bean,由于工厂Bean获取的是FactoryBean本身
				if (isFactoryBean(beanName)) {
	...
	/**
	 * 解决依赖的核心逻辑,针对一个依赖的属性,如构造器参数,方法参数或者字段依赖
	 *
	 * @param descriptor
	 * @param beanName
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			if (value != null) {
				...
				// 执行类型转换,由于注入的值是字符串,但是类型可以为数值类型或者字符串,布尔类型等,通过转换器将对应的值注入到类型中
			...
			// 获取到容器中所有的匹配到的Bean的集合
			Map<String, Object> matchingBeans = findAutowireCandidates(beanName, type, descriptor);
			// 若未在容器中找到
			if (matchingBeans.isEmpty()) {
				// 并且校验是否需要注入,若需要注入,则报错,否则就不进行注入.
			...
			if (instanceCandidate instanceof Class) {
				// 若当前的 值为class对象,表示当前的bean还没有进行初始化,则需要进行创建bean.
				// 此处进入递归的解决依赖.
