...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		// 尝试通过 singleton 缓存中进行查找,getSingleton方法是由父类中DefaultSingletonBeanRegistry 继承下来
		// 当在属性注入时,此处就用于解决循环依赖.
		...
		if (sharedInstance != null && args == null) {
			...
			// 原型 scope 类型的Bean 在创建中
			if (isPrototypeCurrentlyInCreation(beanName)) {
				throw new BeanCurrentlyInCreationException(beanName);
			}

			// Check if bean definition exists in this factory.
			// 若是存在父容器,并且当前BeanDefinition 中不存在该bean名称的定义,则尝试从父容器中获取.
			...
			// 若是只检查类型,不创建,表示该bean不需要创建,则不进行创建bean
			if (!typeCheckOnly) {
				// 标记当前Bean 已经被创建了
			...
			try {
				...
				// 保证当前 BeanDefinition 依赖的Bean先进行初始化和加载
